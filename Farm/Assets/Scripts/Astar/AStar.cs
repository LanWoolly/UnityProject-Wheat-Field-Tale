    ...
    public class AStar : Singleton<AStar>
    {
        ...
        public void BuildPath(string sceneName, Vector2Int startPos, Vector2Int endPos, Stack<MovementStep> npcMovementStack)
        {
            ...
            if (GenerateGridNodes(sceneName, startPos, endPos))
            {
                ...
                {
                    //构建NPC移动路径
                    UpdatePathOnMovementStack(sceneName, npcMovementStack);
                }
            }
        }
        ...
        {
            if (GirdMapManager.Instance.GetGridDimensions(sceneName, out Vector2Int gridDimensions, out Vector2Int gridOrigin))
            {
            ...
            //gridNodes的范围是从0，0开始，所有需要减去原点坐标得到实际位置
            ...
            for (int x = 0; x < gridWidth; x++)
            {
                for (int y = 0; y < gridHeight; y++)
                {
                    ...
                    //没办法跨场景使用
                    // TileDetails tile = GirdMapManager.Instance.GetTileDetailsOnMousePosition(tilePos);

                    var key = tilePos.x + "x" + tilePos.y + "y" + sceneName;
                    TileDetails tile = GirdMapManager.Instance.GetTileDetails(key);
        ...
        /// <summary>
        /// 找到最短路径所有的node添加到closedNodeList
        /// </summary>
        /// <returns></returns>
        ...
        private void UpdatePathOnMovementStack(string sceneName, Stack<MovementStep> npcMovementStep)
        {
